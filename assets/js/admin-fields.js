...
jQuery( function( $ ) {
    ...
    /**
      ...
      e.preventDefault();
		...
		/*$.ajax({
		 ...
		 dataType : "json",
		 url : ajaxurl,
		 data : {action: "get_product_attr", product_id: $('#post_ID').val(), nonce: $("#_wpnonce").val()},
		 success: function(response) {	
			/*
			response = {
				status: "success", 
				data:{
					ids: [4, 14, 20],
					keys:["pa_awning-fabric-colors", "pa_awning-width", "pa_awning-crank-type"],
					names: ["Awning Fabric Color", "Awning Width", "Awning Crank Type"],
				}
			}			
			*//*
			if(response.status == "success") {
			   console.log("--------success---------",response);
			   var str = "<optgroup label='Custom Attributes' id='attributes_group'>";
			   response.data.names.forEach(function(item, index){
				   str+="<option class='custom_attribute' data-type='text' id='custom_attribute_" + index + "' value='" + response.data.keys[index] + "'>" + item + "</option>";
			   });
			   str+="</optgroup>";
			   $(clone_condition).find('.pewc-condition-field').append(str);
			}
			else {
		...
		//$(clone_condition).find('.pewc-condition-field').append("<optgroup label='Custom Attributes' id='attributes_group'><option class='custom_attribute' value=''>attributes</option></optgroup>");
    },
  ...
  function update_conditional_fields() {
	...
	var all_attributes = {};
	if( $( '.product-extra-group-data' ).attr( 'data-attributes' ) ) {      
	   all_attributes = JSON.parse( $( '.product-extra-group-data' ).attr( 'data-attributes' ) );
    }
    ...
    options += '<optgroup class="asdasd" id="cost-optgroup" label="Product Cost">';
	...
	$.ajax({
		...
		success: function(response) {			
			if(response.status == "success") {
			   console.log("--------success---------",response);
			   options += "<optgroup label='Custom Attributes' class='attributes_group'>";
			   response.data.names.forEach(function(item, index){
				   options+="<option class='custom_attribute' data-type='text' id='custom_attribute_" + index + "' value='" + response.data.keys[index] + "'>" + item + "</option>";
			   });
			   options+="</optgroup>";
			   //$(clone_condition).find('.pewc-condition-field').append(str);
			
				// -----------------Update the new condition select field
				var check_exist = $( '.new-conditional-row .pewc-condition-field.pewc-condition-select' ).find('.attributes_group').length;
				if(!check_exist)
					$( '.new-conditional-row .pewc-condition-field.pewc-condition-select' ).append( options );
				console.log($( '.new-conditional-row .pewc-condition-field.pewc-condition-select' ));
  ...
  function pewc_add_value_field( field, field_id, field_type, value_field, val ) {
	  console.log($(".product-extra-group-data").attr('data-fields'));
	  console.log($(".product-extra-group-data").attr('data-attributes'));
		if( val == '__checked__' ) {
			val = '';
		}
