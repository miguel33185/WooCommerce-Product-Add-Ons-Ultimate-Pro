...
<div class="aaaaaa product-extra-conditional-row product-extra-action-match-row" <?php echo $style; ?>>
...
if( ! empty( $item['condition_field'] ) ) {
	...
	foreach( $item['condition_field'] as $index=>$condition ) { ?>
		...
		<div class="product-extra-conditional-row product-extra-conditional-rule" data-condition-count="<?php echo esc_attr( $condition_count ); ?>">
			...
			<div class="product-extra-field-third">
				...
				/*$fields = Array
						(
							[not-selected] =&gt;  -- Select a field -- 
							[pa_awning-fabric-colors] =&gt; Awning Fabric Color
							[pa_awning-width] =&gt; Awning Width
							[pa_awning-crank-type] =&gt; Awning Crank Type
							[cost] =&gt; Cost
							[quantity] =&gt; Quantity
						)
						*/
				...
				if( $field == 'cost' ) {
					...
					//$condition_field_types = get_post_meta( $item_key, 'condition_field', true );					
					$condition_field_type = "custom_field_text";
				}
...
?>
			...
			<div class="product-extra-field-half product-extra-field-last pewc-condition-value-field">
				...
				if( $condition_field_type == 'text' || $condition_field_type == 'custom_field_text') { ?>
				...
				<?php } 
				else if( $condition_field_type == 'select' || $condition_field_type == 'select-box' || $condition_field_type == 'radio' || $condition_field_type == 'image_swatch' || $condition_field_type == 'products' || $condition_field_type == 'checkbox_group' ) { ?>
