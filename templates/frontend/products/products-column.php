			...
			if( $variants ) {
				...
				foreach( $variants as $variant_id ) {
					...
					$variant_wrapper .= sprintf(
						...
						esc_attr( number_format( floatval( $variant_price ), get_option( 'woocommerce_price_num_decimals', 2 ) ) ),
	  ...
	  $name = sprintf(
		...
		if( $products_quantities == 'independent' ) {
			...
			$quantity_field = sprintf(
				'<span class="pewc-quantity-wrapper"><input type="number" min="0" step="1" max="%s" class="pewc-form-field pewc-child-quantity-field" name="%s" value="%s" %s></span>',
				$available_stock,
