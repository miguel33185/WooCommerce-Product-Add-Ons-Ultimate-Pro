...
/**
 * Get all the fields on the page which will trigger a field condition
 * Find all the fields on the page, then map those IDs to fields with conditions
 * @since 3.9.0
 * @return Array
 */
...
function pewc_get_adjusted_product_addon_price( $cart_item, $cart_key ) {
	...
	if( apply_filters( 'pewc_ignore_tax_adjustments', false, $cart_item ) ) {
		return $new_price;
	}
	...
	if( $adjust_tax == 'remove' ) {
		...
		$original_price = $cart_item['product_extras']['original_price'];
		$original_extras = $new_price - $original_price;
		$original_extras = pewc_get_price_without_tax( $original_extras, $cart_item['data'] );
