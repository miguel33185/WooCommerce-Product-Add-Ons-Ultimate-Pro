...
function pewc_get_all_fields( $group=false, $is_ajax=false, $product_id=false ) {
	...
	if( $is_ajax || ( isset( $_GET['post'] ) && get_post_type( $_GET['post'] ) == 'product' ) ) {
		...
		foreach ($_attributes as $attr=>$attr_deets) {			
			$fields[ $attr ] = wc_attribute_label($attr);
		}
	...
	<script>
		jQuery(document).ready(function($) {
							...
							if( ! empty( $conditions ) ) {
								...
								echo "function pewc_check_rules_for_{$id}( field_value ) {\n";
									...
									foreach( $conditions as $condition ) {
										...
										} else if( strpos($condition['field'],'pa_')===0 ) {
											echo "field_value_{$rule_count} = $('#{$condition['field']} option:selected').text();\n";
										}
										...
										if( $condition['field'] == 'cost' || $condition['field'] == 'quantity' ) {
											...
											echo "console.log('field_value_{$rule_count}='+field_value_{$rule_count})\n";
											echo "console.log('condition_value_{$rule_count}='+condition_value_{$rule_count})\n";
										...
										// Don't bother to check subsequent conditionss
								...
								foreach( $conditions as $condition ) {
									...
									if( $condition['field'] == 'cost' ) {
										...
										if( strpos($condition['field'],'pa_')===0 ) {
											echo "$( 'body' ).on( 'change', '#{$condition['field']}, .{$condition['field']} .pewc-radio-form-field', function( e ) {\n
			...
			function pewc_check_all_conditions( id, conditions_met, action, match ) {
				console.log('pewc_check_all_conditions=> id='+id+', conditions_met='+conditions_met+', action='+action+', match='+match);
				if( match == 'all' ) {
					var all_met=true;
			...
			function pewc_conditions_met( id, action ) {
				...
				console.log('pewc_conditions_met=>id='+id+', action='+action);
			...
			function pewc_conditions_not_met( id, action ) {
				...
				console.log('pewc_conditions_not_met=>id='+id+', action='+action);
				// Use this to retrigger calculations
				$( 'body' ).trigger( 'pewc_field_visibility_updated', [ id, action ] );
			}
